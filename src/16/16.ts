...
const SplitterVert: [DirIdx[], DirIdx[], DirIdx[], DirIdx[]] = [[1, 3], [1], [1, 3], [3]]
const SplitterHorz: [DirIdx[], DirIdx[], DirIdx[], DirIdx[]] = [[0], [0, 2], [2], [0, 2]]

function addCoord ([i1, j1]: Coord, [i2, j2]: Coord): Coord {
  return [i1 + i2, j1 + j2]
}
...
class Problem {
  ...
  cleanEnergized (): void {
    this.energized = Array(this.maxI).fill(0).map((_zero) => {
      return Array(this.maxJ).fill(0).map((_zero) => [false, false, false, false])
    })
  }
  ...
  edgeRays (): Array<[DirIdx, Coord]> {
    const rowRays: Array<[DirIdx, Coord]> = [...Array(this.maxI).keys()].flatMap((row) => [[0, [row, 0]], [2, [row, this.maxJ - 1]]])
    const colRays: Array<[DirIdx, Coord]> = [...Array(this.maxJ).keys()].flatMap((col) => [[1, [this.maxI - 1, col]], [3, [0, col]]])
